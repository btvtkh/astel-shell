...
def BluetoothQSB():
    ...
    toggle_button = Widget.get_child_by_name(ret, "bluetooth-qsb-toggle-button")
    reveal_button = Widget.get_child_by_name(ret, "bluetooth-qsb-reveal-button")
    status_label = Widget.get_child_by_name(ret, "bluetooth-qsb-status-label")
    separator = Widget.get_child_by_name(ret, "bluetooth-qsb-separator")

    def on_adapter_powered(*_):
        if adapter and adapter.get_powered():
            status_label.set_label("Enabled")
            ret.add_css_class("toggled")
        else:
            status_label.set_label("Disabled")
            ret.remove_css_class("toggled")
